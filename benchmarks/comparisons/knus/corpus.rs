...
// reject. Time is measured regardless; rejection speed is part of
// real-world throughput. knus's error reporting on malformed input
// is slower than its accept-path — that asymmetry is real.
//
// Usage: knus-corpus <corpus-dir>
use std::env;
use std::fs;
...
fn main() {
    let dir = match env::args().nth(1) {
        ...
        None => { eprintln!("usage: knus-corpus <corpus-dir>"); std::process::exit(2); }
    };
    if !dir.is_dir() {
    ...
    for _ in 0..ITERS {
        ...
        for (name, content) in &files {
            let r: Result<knus::ast::Document<knus::span::Span>, _> =
                knus::parse_ast(name.as_str(), content);
    ...
    println!("  knus    corpus  files={}  bytes={}  iters={}  us/file={:.2}  files/s={:.1}K  KB/s={:.0}  ok={}/{}",
